...
function formatDate(dateStr) {
  if (!dateStr) return '';
  // If it's ISO format like 2024-01-15T00:00:00 or similar, extract date part
...
export default function UploadForm({ onUpload }) {
        ...
        if (i === index) {
          return {
            ...
            result: {
              ...
              namaPt: doc.nama_pt || '',
              tanggalSurat: formatDate(doc.tanggal_surat || ''),
    ...
    setQueue(prev => prev.map((item, i) => {
      if (i === index) {
        return {
          ...
          editing: true,
          editData: {
            namaPt: item.result?.namaPt || '',
            tanggal: item.result?.tanggalSurat || '',
          },
        };
      }
      return item;
  ...
  const handleEditChange = (index, field, value) => {
    setQueue(prev => prev.map((item, i) =>
      i === index ? { ...item, editData: { ...item.editData, [field]: value } } : item
    ));
  };
  ...
  const handleSaveEdit = async (index) => {
    ...
    try {
      await api.updateDocument(item.result.id, {
        nama_pt: item.editData.namaPt || undefined,
        tanggal_surat: item.editData.tanggal || undefined,
      });
      ...
      setQueue(prev => prev.map((qItem, i) => {
        if (i === index) {
          return {
            ...
            result: {
              ...qItem.result,
              namaPt: item.editData.namaPt || qItem.result.namaPt,
              tanggalSurat: item.editData.tanggal || qItem.result.tanggalSurat,
            },
          };
        }
        return qItem;
        ...
        >
          ...
          <div className="flex flex-col items-center gap-3">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${
              dragActive ? 'bg-zinc-200' : 'bg-zinc-100'
            }`}>
              <Upload className={`w-6 h-6 ${dragActive ? 'text-zinc-900' : 'text-zinc-400'}`} />
            </div>
            <div>
              <p className="text-sm font-medium text-zinc-700">
                <span className="text-zinc-900 font-semibold">Klik untuk upload</span> atau drag & drop
              </p>
          ...
          <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden shadow-sm">
            ...
            <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {processing && (
                  <span className="text-xs text-zinc-500 font-medium ml-2">Memproses...</span>
                )}
              </div>
              <div className="flex items-center gap-2">
                {allDone && hasResults && (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      ...
                      {item.status === 'uploading' && (
                        <span className="text-xs text-zinc-600 px-2 py-1 rounded bg-zinc-100 flex items-center gap-1">
                      ...
                      {item.status === 'done' && (
                        <span className="text-xs text-zinc-700 px-2 py-1 rounded bg-zinc-100 flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3" />
                          Selesai
                        </span>
                      )}
                      {item.status === 'error' && (
                        <span className="text-xs text-red-700 px-2 py-1 rounded bg-red-50 flex items-center gap-1">
                          <X className="w-3 h-3" />
                          Error
                        </span>
                      )}
                    ...
                    <div className="mt-3 space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-zinc-500 font-medium w-10 text-right">
                          {Math.min(Math.round(item.progress), 100)}%
                        </span>
                      </div>
                      <p className="text-xs text-zinc-400">
                        {item.status === 'uploading' ? 'Mengupload file...' : 'Menganalisis dokumen...'}
                      </p>
                    </div>
                  )}
                    ...
                    <div className="mt-4 pt-4 border-t border-zinc-100 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="p-3 rounded-lg bg-zinc-50 border border-zinc-200">
                          ...
                          <p className="text-sm font-semibold text-zinc-900">{item.result.arah}</p>
                        </div>
                        <div className="p-3 rounded-lg bg-zinc-50 border border-zinc-200">
                          <p className="text-xs text-zinc-500 mb-1">Jenis Dokumen</p>
                          <p className="text-sm font-semibold text-zinc-900">{item.result.jenis}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <div className="p-3 rounded-lg bg-zinc-50 border border-zinc-200">
                          <p className="text-xs text-zinc-500 mb-1">Hasil</p>
                        ...
                        <div className="p-3 rounded-lg bg-zinc-50 border border-zinc-200">
                          ...
                          {item.editing ? (
                            <input
                              type="text"
                              value={item.editData.namaPt}
                              onChange={(e) => handleEditChange(index, 'namaPt', e.target.value)}
                              className="w-full text-xs font-medium text-zinc-900 bg-white border border-zinc-300 rounded px-2 py-1 outline-none focus:border-zinc-500"
                            />
                          ) : (
                        ...
                        <div className="p-3 rounded-lg bg-zinc-50 border border-zinc-200">
                          ...
                          {item.editing ? (
                            <input
                              type="date"
                              value={item.editData.tanggal}
                              onChange={(e) => handleEditChange(index, 'tanggal', e.target.value)}
                              className="w-full text-xs font-medium text-zinc-900 bg-white border border-zinc-300 rounded px-2 py-1 outline-none focus:border-zinc-500"
                            />
                          ) : (
                      ...
                      <div className="flex items-center gap-2">
                        {item.editing ? (
                          ...
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100 rounded-lg transition-colors border border-zinc-200"
                          >
                      ...
                      {(!item.result.extractedText || item.result.extractedText.length < 10) && (
                        <div className="p-2 rounded-lg bg-amber-50 border border-amber-200 flex items-start gap-2">
                          ...
                          <p className="text-xs text-amber-800">Teks gagal diekstrak. Silakan klik Koreksi untuk mengisi data secara manual.</p>
                        </div>
                      )}
                    </div>
                  )}
