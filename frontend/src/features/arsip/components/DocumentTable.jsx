...
import { FileText, Eye, Trash2, ArrowDownLeft, ArrowUpRight, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function DocumentTable({ documents, total, page, totalPages, onPageChange, onDelete, onUpdate, onSelect, selected }) {
  ...
  const handleDelete = async (id) => {
    ...
    if (detailDoc && detailDoc.id === id) setDetailDoc(null);
  };

  const handleUpdate = (id, data) => {
    onUpdate(id, data);
    if (detailDoc && detailDoc.id === id) setDetailDoc(prev => ({ ...prev, ...data }));
  };
          ...
          {documents.length === 0 ? (
            ...
            documents.map((doc) => {
              ...
              return (
                <div
                  key={doc.id}
                  className="grid grid-cols-[40px_1fr_160px_120px_80px_120px] gap-2 px-4 py-2.5 border-b border-zinc-50 hover:bg-zinc-50/50 transition-colors group cursor-pointer"
                ...
                >
                  ...
                  <div className="flex items-center justify-end pr-2 gap-1">
                    ...
                    <div className={`flex items-center gap-0.5 transition-opacity ${isHovered ? 'opacity-100' : 'opacity-0'}`} onClick={e => e.stopPropagation()}>
