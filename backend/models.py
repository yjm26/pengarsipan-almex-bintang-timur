...
class User(Base):
    ...
    role = Column(String(20), default="admin_dokumen")  # admin_dokumen / owner
...
class AIModel(Base):
    ...
    created_at = Column(DateTime, default=now_wib)
    # Metrics terpisah (JSON string)
    arah_metrics_json = Column(Text, default="")
    jenis_metrics_json = Column(Text, default="")
    train_size = Column(Integer, default=0)
    test_size = Column(Integer, default=0)
    split_note = Column(Text, default="")
